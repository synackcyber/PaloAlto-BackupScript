...
curl "https://PALO-DNS-NAME/api/?type=export&category=device-state&key=PALO-API-KEY" -OutFile ("$destDevice\GV_DeviceStateConfiguration" + "$Date" +".xml")
Start-Sleep -Seconds 10

## Download running config:
curl "https://PALO-DNS-NAME/api/?type=export&category=configuration&key=PALO-API-KEY" -OutFile ("$destRunning\GV_RunningConfiguration" + "$Date" +".xml")
Start-Sleep -Seconds 10
